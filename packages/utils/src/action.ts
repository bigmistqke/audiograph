...
export type ActionGenerator<R> = Generator<
  Promise<unknown>,
  R | HoldMarker<R>,
  unknown
>;
...
function isPromiseWithCleanup<T>(
  value: unknown,
): value is PromiseWithCleanup<T> {
 ...
 * ```
 */
export function action<T = undefined, R = void>(
...
action.phase = function <Name extends string, Result>(
  ...
  return createPhaseBuilder<Name, Result>([
    { name, fn: fn as PhaseFn<unknown, unknown> },
  ]);
};
